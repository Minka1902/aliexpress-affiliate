...
// Lazy-load the heavy recharts bundle; render a skeleton until it's ready.
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Instant paint from the last session's data while we refresh in the background.
    try {
    ...
    (async () => {
      try {
        ...
        if (!active) return;
        const o = Array.isArray(ordersData.orders) ? ordersData.orders : [];
        const p = Array.isArray(recsData.products) ? recsData.products : [];
        setOrders(o);
        setSummary(ordersData.summary ?? null);
        setProducts(p);
        try {
          sessionStorage.setItem(CACHE_KEY, JSON.stringify({ orders: o, summary: ordersData.summary ?? null, products: p }));
        } catch {
          /* ignore */
        }
      } catch {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Charts (recharts loaded lazily) */}
      <DashboardCharts categoryData={categoryData} statusData={statusData} overTimeData={overTimeData} />
